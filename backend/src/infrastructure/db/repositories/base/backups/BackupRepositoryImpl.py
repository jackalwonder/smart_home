...
class BackupRepositoryImpl:
                        ...
                        await session.execute(
                            text(
                                ...
                                SELECT
                                    ...
                                    y,
                                    icon_type,
                                    icon_asset_id::text AS icon_asset_id,
                                    label_mode,
                                    is_visible,
                                    structure_order,
        ...
        return {
            "settings": {
                ...
                "page_settings": {
                    ...
                    "homepage_display_policy": as_dict(
                        page_settings["homepage_display_policy_json"]
                    ),
                ...
                "function_settings": {
                    ...
                    "offline_threshold_seconds": int(
                        function_settings["offline_threshold_seconds"]
                    ),
                    "quick_entry_policy": as_dict(
                        function_settings["quick_entry_policy_json"]
                    ),
                    ...
                    "auto_home_timeout_seconds": int(
                        function_settings["auto_home_timeout_seconds"]
                    ),
            ...
            "layout": {
                ...
                "layout_meta": as_dict(layout_row["layout_meta_json"])
                if layout_row is not None
                else {},
                "hotspots": hotspots,
            },
    ...
    async def create_backup(self, row: BackupCreateRow) -> None:
        ...
        async with session_scope(self._database) as (session, owned):
            await session.execute(stmt, row.__dict__)
            if owned:
    ...
    async def list_backups(self, home_id: str) -> list[BackupListRow]:
        ...
        return [BackupListRow(**dict(row)) for row in rows]
