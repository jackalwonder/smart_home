...
- ==terminal_id==（仅用于兼容传输或诊断定位；若传递，必须与 access token claim 一致）  
- ==home_id==（仅用于兼容传输或诊断定位；若传递，必须与 access token claim 一致）  
- 家庭与终端权威上下文以 ==access_token== 中的 ==home_id / terminal_id== claim 为准  
...
1. ==sequence== 仅在单连接内有效。  
2. 断线重连时，前端应携带 ==last_event_id==，服务端优先执行增量回放。  
3. 若 ==last_event_id== 缺失、失效、超出 outbox 补偿窗口，或 ==snapshot_required = true==，前端必须立即补拉对应接口。  
4. 若检测到 ==sequence== 不连续，前端应进入补偿流程。  
5. 快照补偿是重连补偿兜底，不替代 ==last_event_id== 增量回放优先级。  
